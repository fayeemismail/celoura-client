...
const categories = [
  ...
  { id: 3, name: "City", icon: <Building2 className="w-8 h-8 text-gray-700" /> },
  { id: 4, name: "Royal", icon: <Landmark className="w-8 h-8 text-yellow-600" /> },
  { id: 5, name: "Adventure", icon: <PlaneTakeoff className="w-8 h-8 text-red-500" /> },
  { id: 6, name: "Foods", icon: <Utensils className="w-8 h-8 text-orange-500" /> }
];
    ...
    <section className="mb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
        {categories.map((category) => (
          <div
            key={category.id}
            onClick={() => navigate(`/destinations?search=${category.name}`)}
            style={{
